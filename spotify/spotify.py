...
Self-contained (Python stdlib only). Auto-loads .env, so you can just run e.g.
`python3 spotify.py me` without exporting anything first.
...
Env vars (from .env, see SKILL.md):
  ...
  SPOTIFY_SCOPES         (optional; default: playlist-modify-public playlist-modify-private)

The user token (incl. refresh token) is cached in .spotify_token.json next to this
file and refreshed automatically. The FIRST run needs interactive authorization:
the script prints `AUTH_URL <url>` and waits on the local callback server — open
that URL in a browser logged into the right Spotify account (drive Playwright to it).

Commands:
  auth                                          (re)authorize and cache a token
  token                                         print a valid access token (refresh if needed)
...
def env(name, default=None, required=False):
    ...
    if required and not v:
        sys.exit(f"Missing required env var {name} — copy .env.example to .env and fill it in.")
    return v
...
# Default: the full set of user-facing Web API scopes, so the token is never the
# limiting factor (override with SPOTIFY_SCOPES). Editing playlists only strictly
# needs playlist-modify-*, but a broad token avoids re-auth when other calls are added.
...
def _post_token(data):
    req = urllib.request.Request(
        ACCOUNTS + "/api/token", data=urllib.parse.urlencode(data).encode(),
...
def _save_token(tok):
    ...
    if "refresh_token" not in tok and os.path.exists(TOKEN_FILE):
        try:  # refresh responses usually omit the refresh_token — keep the old one
            tok["refresh_token"] = json.load(open(TOKEN_FILE)).get("refresh_token")
        except Exception:
            pass
...
def interactive_auth():
    ...
    srv.serve_forever()
    if "code" not in got:
        sys.exit("Authorization failed: " + got.get("error", "no code returned"))
    tok = _save_token(_post_token({"grant_type": "authorization_code",
...
def _refresh(refresh_token):
    new = _post_token({"grant_type": "refresh_token", "refresh_token": refresh_token})
    new.setdefault("refresh_token", refresh_token)  # Spotify omits it on refresh — keep ours
    return _save_token(new)


def get_token(force=False):
    """valid cached access token -> refresh (env SPOTIFY_REFRESH_TOKEN, else cached) ->
    interactive browser auth (one-time bootstrap)."""
    tok = None
    if not force and os.path.exists(TOKEN_FILE):
        try:
            ...
            tok = None
        if tok and tok.get("access_token") and tok.get("expires_at", 0) > time.time():
            return tok
    if not force:
        rt = os.environ.get("SPOTIFY_REFRESH_TOKEN") or (tok.get("refresh_token") if tok else None)
        if rt:
            try:
                return _refresh(rt)
    ...
    while True:
        try:
            ...
            if e.code == 429:
                ...
                time.sleep(wait)
                continue
            sys.exit(f"API {e.code} on {method} {path}: {e.read().decode()}")
...
def need_playlist(arg):
    pid = arg or DEFAULT_PLAYLIST
    if not pid:
        sys.exit("No playlist id — pass one or set SPOTIFY_PLAYLIST_ID in .env.")
    return spotify_id(pid, "playlist")
...
def cmd_add_tracks(a):
    ...
    _add(pid, to_add)
    pl = api("GET", f"/playlists/{pid}", {"fields": "tracks(total),name"})
    print(f"Added {len(to_add)} tracks ({skipped} skipped). "
          f"'{pl['name']}' now has {pl['tracks']['total']}.")
